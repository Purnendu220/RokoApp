...
import com.rokoapp.utils.GPSTracker;
...
public class TrackMyBus extends AppCompatActivity implements OnMapReadyCallback, ResponseProgressListener, View.OnClickListener {
    ...
    List<RouteStopData.Data> stopsList = new ArrayList<>();
    ...
    List<RouteStopData.Data> mUserStopsList = new ArrayList<>();
    int originStopId,mDestinationStopId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_track_my_bus_one);
        initViews();
        ...
        ApiManager.getRouteStopList(TrackMyBus.this,virtualRouteId,this);
        if (toolbar != null) {
    ...
    private void showData() {
        ...
        RouteStopsAdapter adapterImage = new RouteStopsAdapter(TrackMyBus.this, mUserStopsList);
        ...
        int listCount = mUserStopsList.size();
        if (listCount==0){
    ...
    public void onResponseCompletedWithData(int i, Object data) {
        if(data!=null&&data instanceof RouteStopData){
            RouteStopData response = (RouteStopData) data;
            if(response!=null&&response.getData()!=null){
                stopsList =response.getData();
                setupStartAndEndPoint();
                showData();

                mMap.moveCamera(CameraUpdateFactory.newLatLngZoom((LatLng) markerPoints.get(0), 16));
    ...
    private void setupStartAndEndPoint(){
        ...
        for(int i=0;i<stopsList.size();i++){
            RouteStopData.Data model=stopsList.get(i);
            LatLng position = new LatLng(model.getLatitude(), model.getLongitude());
            if(model.getBus_id()==originId||model.getBus_id()==destinationId){
// Adding new item to the ArrayList
                ...
                if (model.getBus_id()==originId) {
                    options.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN));
                    options.title(userRouteData.getOriginStationName());
                    originStopId=model.getId();
                    markerPoints.add(0,position);




                } else if (model.getBus_id()==destinationId) {
                    options.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED));
                    options.title(userRouteData.getDestinationStationName());
                    mDestinationStopId=model.getId();
        ...
        for(int i=0;i<stopsList.size();i++){
            ...
            if(model.getId()==originStopId||model.getId()==mDestinationStopId) {
                mUserStopsList.add(model);
...
// Drawing polyline in the Google Map for the i-th route
    ...
    private void setMarker(RouteStopData.Data model) {
